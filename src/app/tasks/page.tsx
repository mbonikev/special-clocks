...
import { useTaskSync } from "@/hooks/use-db-sync";
import { PageShell } from "@/components/page-shell";

export default function TasksPage() {
  const { tasks, activeTaskId, setActiveTask } = useAppStore();
  const { add, toggle, remove } = useTaskSync();
    ...
    <PageShell className="h-full flex flex-col">
      ...
      >
        <div className="mx-auto w-full max-w-2xl flex items-center justify-between">
          <div>
            <h1 className="text-base font-semibold tracking-tight" style={{ color: "var(--text)" }}>Tasks</h1>
      ...
      >
        <div className="mx-auto w-full max-w-2xl flex gap-2">
          ...
          <div className="flex items-center gap-1">
            <span className="text-xs" style={{ color: "var(--text-subtle)" }}>Est:</span>
            <select
              value={estimate}
            ...
            >
              {[1, 2, 3, 4, 5, 6, 7, 8].map((n) => (
                <option key={n} value={n} style={{ background: "var(--surface)" }}>{n}</option>
              ))}
            </select>
          </div>
          <button
                      ...
                      <div className="flex gap-0.5">
                        {Array.from({ length: task.pomodorosEstimate }).map((_, i) => (
                          <div
                            ...
                            style={{ background: i < task.pomodorosDone ? "var(--accent)" : "var(--bg-muted)" }}
                          />
                        ))}
                      </div>
                    </div>
      ...
      {tasks.length > 0 && (
        <div
          className="px-6 py-2 border-t flex items-center shrink-0 text-xs"
          style={{ borderColor: "var(--border)", color: "var(--text-subtle)", background: "var(--surface)" }}
        >
          <div className="mx-auto w-full max-w-2xl flex items-center">
            <span>{tasks.reduce((a, t) => a + t.pomodorosDone, 0)} pomodoros logged</span>
            <button
              onClick={() => tasks.filter((t) => t.done).forEach((t) => remove(t.id))}
